...
# Usage:
# ./generate_thomson_dat.py > "../dat/Thomson - MOTO.dat"
...
# Returns the content of the requested URL, with several retries if needed.
...
def get_screenscraper_rom_letters(platformId):
    ...
    page = download_with_retries(base_url)
...
def get_screenscraper_rom_by_id(platformId, romid):
    ...
    page = download_with_retries(url)
...
def print_dat_rom(rom):
    print('Downloaded: ' + rom.gamename, file=sys.stderr)
    print('')
    print('game (')
...
def print_dat_header():
    ...
    print('\thomepage "https://github.com/Zlika/libretro-database-thomson"')
